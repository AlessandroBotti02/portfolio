...
type ToolDef = {
  ...
  name: string;
  short?: string;
  svgFile?: string;   // path under /icons/
  iconSize?: number;  // override default 32px
  bg: [string, string];
  level: 1 | 2 | 3 | 4 | 5;
  levelEn: string;
  levelIt: string;
...
function AppIcon({ tool, globalIndex, lang, inView }: {
  ...
  return (
    ...
    >
      {/* Tooltip — centered with Framer Motion x */}
      <AnimatePresence>
        {hovered && (
          <motion.div
...
export default function Stack({ lang }: { lang: "en" | "it" }) {
  ...
  // Build global index for stagger across all groups
  let globalIndex = 0;
